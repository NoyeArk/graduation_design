...
#            self.drop_user,self.drop_item = 3,3
        ...
        self.dict_entity2id['user'], self.dict_entity2id['item'] = self._get_rating(
            remove_rating,
            self.drop_user,
            self.drop_item
        )  # 53
        ...
        try:
            # 获取每个用户最近的交互记录
            self.latest_interaction = self.find_latest_interaction(
                self.dict_list['user_item'],
                args.maxlen
            )  # 时间特征
    ...
    def _get_rating(self, score, remove_less_than1, remove_less_than2, keep_interaction=-1):
        ...
        Args:
            score (`float`): 评分阈值
            remove_less_than1 (`int`): 用户评分阈值
            remove_less_than2 (`int`): 物品评分阈值
        ...
        for u, count in users_cnt.items():
            if count >= remove_less_than1:
                user_list.append(u)

        # 统计每个物品出现的次数
        items_cnt = Counter(items)
        for item, count in items_cnt.items():
            if count >= remove_less_than2:
