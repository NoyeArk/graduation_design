...
#            self.drop_user,self.drop_item = 3,3
       ...
       #save 
        ...
        try:
            self.latest_interaction = self.find_latest_interaction(self.dict_list['user_item'],args.maxlen) #时间特征
        except:
            self.latest_interaction = self.find_latest_interaction(self.dict_list['user_item'],5) #时间特征
