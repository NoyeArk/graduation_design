...
class EnsRecGate(nn.Module):
    def __init__(self, args, data_args, n_user, n_item):
        super(EnsRecGate, self).__init__()
        self.args = args
        ...
        self.item_embeddings = nn.Embedding(n_item, self.hidden_dim)
        ...
        self.basemodel_emb = nn.Parameter(torch.randn(1, self.n_base_model, self.hidden_dim))

        self.prefer_gate = nn.Sequential(
            nn.Linear(2 * self.hidden_dim, self.hidden_dim),  # 增加隐藏层维度
            nn.ReLU(),
            nn.Dropout(self.args['dropout_rate']),  # 添加dropout层
        ...
        self.basemodel_gate = nn.Linear(2*self.hidden_dim, self.n_base_model)
    ...
    def forward(self, batch, is_test=False):
        ...
        Args:
            ...
            - user_id (`torch.Tensor`): 用户ID，形状为 [bc]
            - user_seq (`torch.Tensor`): 用户序列，形状为 [bc, seq_len]
            - pos_item (`torch.Tensor`): 物品ID，形状为 [bc]
            - neg_item (`torch.Tensor`): 负样本物品ID，形状为 [bc]
            - pos_label (`torch.Tensor`): 所有物品的得分，形状为 [bc]
            - neg_label (`torch.Tensor`): 所有物品的得分，形状为 [bc]
            - base_model_preds (`torch.Tensor`): 基模型预测，形状为 [bc, k, 100]
        ...
        user_interaction1 = self.item_embeddings(batch['user_seq'])
        target_emb1 = self.item_embeddings(batch['pos_item'])  # bc, dim
        preference1 = self.dien(user_interaction1, target_emb1) + user_emb  # bc, dim

        preference_gate_input = torch.cat([preference, preference1], dim=-1)  # [bc, dim * 2]
        preference_gate = self.prefer_gate(preference_gate_input)  # [bc, 1]
        preference = preference_gate * preference + (1 - preference_gate) * preference1  # [bc, dim]
        ...
        # # self.base_model_emb [1, n_base_model, dim]

        # gate_input = torch.cat([basemodel_emb.mean(dim=1), self.base_model_emb.squeeze(0).mean(0)], dim=-1)  # [bc, dim * 2]
        # gate = torch.sigmoid(self.basemodel_gate(gate_input))  # [bc, n_base_model]
        # basemodel_emb = gate.unsqueeze(-1) * basemodel_emb + (1 - gate.unsqueeze(-1)) * self.base_model_emb  # [bc, n_base_model, dim]

        # 计算基模型权重
        # [bc, n_base_model, dim] @ [bc, 1, dim] -> [bc, n_base_model, 1]
        preference = preference.unsqueeze(1).transpose(-2, -1)  # [bc, dim, 1]
        wgts_org = torch.matmul(self.basemodel_emb, preference).squeeze(-1)  # [bc, n_base_model]
        ...
        if self.args['use_div']:
            ...
            model_emb1 = self.basemodel_emb.unsqueeze(1)  # [bc, 1, n_base_model, dim]
            model_emb2 = self.basemodel_emb.unsqueeze(2)  # [bc, n_base_model, 1, dim]
