...
class EnsRec(nn.Module):
    ...
    def forward(self, batch, is_test=False):
        ...
        Returns:
            `dict`: 包含损失和预测结果的字典
        """
        user_emb = self.user_embeddings(batch['user_id'])  # bc, 2*dim
        user_interaction = self.item_embeddings(batch['user_seq'])  # bc, seq_len, dim
        # user_interaction2 = self.item_tower(batch['user_seq'], 'user_seq')  # bc, seq_len, dim
        # user_interaction = torch.cat((user_interaction1, user_interaction2), dim=-1)  # bc, seq_len, 2*dim
        
        target_emb = self.item_embeddings(batch['pos_item'])  # bc, dim
        # target_emb2 = self.item_tower(batch['pos_item'], 'single_item')  # bc, dim
        # target_emb = torch.cat((target_emb1, target_emb2), dim=-1)  # bc, 2*dim
        
        preference = self.dien(user_interaction, target_emb) + user_emb  # bc, 2*dim

        # 计算基模型权重
        # [bc, n_base_model, dim] @ [bc, 1, dim] -> [bc, n_base_model, 1]
        preference = preference.unsqueeze(1).transpose(-2, -1)  # [bc, 2*dim, 1]
